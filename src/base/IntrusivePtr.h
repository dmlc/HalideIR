...
public:
    ...
    bool is_one() const {return count == 1;}
};
...
public:
    ...
    /**
     * check if the handle uniquely holds the object
     * Helpful to implement copy-on-write semnatics
     */
    inline bool unique() const  {
        if (ptr) return ref_count(ptr).is_one();
    }
};
