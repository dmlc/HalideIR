...
public:
    const VarExpr &store_var;
    const int store_stride, max_stores;
    std::vector<Stmt> &let_stmts;
    std::vector<Stmt> &stores;

    StoreCollector(const VarExpr &var, int stride, int ms,
                   std::vector<Stmt> &lets,
                   std::vector<Stmt> &ss) :
        store_var(var), store_stride(stride), max_stores(ms),
...
private:
    ...
    void visit(const For *op, const Stmt &s) {collecting = false; stmt = s;}
    void visit(const IfThenElse *op, const Stmt &s) {collecting = false; stmt = s;}
    void visit(const ProducerConsumer *op, const Stmt &s) {collecting = false; stmt = s;}
    void visit(const Allocate *op, const Stmt &s) {collecting = false; stmt = s;}
    void visit(const Realize *op, const Stmt &s) {collecting = false; stmt = s;}
    ...
    void visit(const Load *op, const Expr &e) {
        ...
        if (equal(op->buffer_var, store_var)) {
            collecting = false;
            expr = e;
        } else {
...
Stmt collect_strided_stores(Stmt stmt, const VarExpr &var, int stride, int max_stores,
                            std::vector<Stmt> lets, std::vector<Stmt> &stores) {

    StoreCollector collect(var, stride, max_stores, lets, stores);
...
private:
    ...
    void visit(const Load *op, const Expr &e) {
        if (op->type.is_scalar()) {
            ...
            expr = Load::make(t, op->buffer_var, mutate(op->index));
        }
    }
    ...
    void visit(const Variable *op, const Expr &e) {
        if (op->type.is_scalar()) {
            ...
            } else if (external_lets.contains(op) &&
                       starting_lane == 0 &&
                       lane_stride == 2) {
                expr = Variable::make(t, op->name_hint + ".even_lanes");
            } else if (external_lets.contains(op) &&
                       starting_lane == 1 &&
                       lane_stride == 2) {
                expr = Variable::make(t, op->name_hint + ".odd_lanes");
            } else if (external_lets.contains(op) &&
                       starting_lane == 0 &&
                       lane_stride == 3) {
                expr = Variable::make(t, op->name_hint + ".lanes_0_of_3");
            } else if (external_lets.contains(op) &&
                       starting_lane == 1 &&
                       lane_stride == 3) {
                expr = Variable::make(t, op->name_hint + ".lanes_1_of_3");
            } else if (external_lets.contains(op) &&
                       starting_lane == 2 &&
                       lane_stride == 3) {
                expr = Variable::make(t, op->name_hint + ".lanes_2_of_3");
            } else {
    ...
    void visit(const Let *op, const Expr &e) {
        if (op->type.is_vector()) {
            ...
            VarExpr new_var = Variable::make(new_type, new_name);
            ...
            // TODO(haichen): what's Halide's logic here?
...
class Interleaver : public IRMutator {
    ...
    void visit(const Block *op) {
...

        {
            ...
            Stmt rest = collect_strided_stores(op->rest, store->buffer_var,
