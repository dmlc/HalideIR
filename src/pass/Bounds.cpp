...
private:
    ...
    void bounds_of_func(FunctionRef func, int value_index, Type t) {
        // if we can't get a good bound from the function, fall back to the bounds of the type.
        bounds_of_type(t);

        pair<FunctionRef, int> key = make_pair(func, value_index);
    ...
    void visit(const IntImm *op, const Expr &e) {
        interval = Interval::single_point(e);
    }

    void visit(const UIntImm *op, const Expr &e) {
        interval = Interval::single_point(e);
    }

    void visit(const FloatImm *op, const Expr &e) {
        interval = Interval::single_point(e);
    }
  ...
  void visit(const Add *op, const Expr &e) {
    ...
    void visit(const Load *op, const Expr &e) {
        ...
        if (interval.is_single_point()) {
            ...
            Expr load_min =
                Load::make(op->type.element_of(), op->buffer_var, interval.min);
    ...
    void visit(const Ramp *op, const Expr &e) {
        ...
        VarExpr var = Variable::make(op->base.type(), var_name);
        Expr lane = op->base + var * op->stride;
        scope.push(var.get(), Interval(make_const(var.type(), 0),
    ...
    void visit(const Call *op, const Expr &e) {
        ...
        if (const_args &&
            ...
            bounds_of_func(op->func, op->value_index, op->type);
        } else {
            // Just use the bounds of the type
            bounds_of_type(t);
        }
    ...
    void visit(const Let *op, const Expr &e) {
        ...
        Interval var;
        VarExpr min_var = Variable::make(op->value.type().element_of(), op->var->name_hint + ".min");
        VarExpr max_var = Variable::make(op->value.type().element_of(), op->var->name_hint + ".max");
        ...
        scope.push(op->var.get(), var);
        op->body.accept(this);
        scope.pop(op->var.get());

        if (interval.has_lower_bound()) {
            if (val.min.defined() && expr_uses_var(interval.min, min_var.get())) {
                interval.min = Let::make(min_var, val.min, interval.min);
            }
            if (val.max.defined() && expr_uses_var(interval.min, max_var.get())) {
                interval.min = Let::make(max_var, val.max, interval.min);
            }
        }

        if (interval.has_upper_bound()) {
            if (val.min.defined() && expr_uses_var(interval.max, min_var.get())) {
                interval.max = Let::make(min_var, val.min, interval.max);
            }
            if (val.max.defined() && expr_uses_var(interval.max, max_var.get())) {
                interval.max = Let::make(max_var, val.max, interval.max);
            }
        }
    }
...
Region region_union(const Region &a, const Region &b) {
    ...
    for (size_t i = 0; i < a.size(); i++) {
        ...
        result.push_back(Range::make_by_min_extent(simplify(min), simplify(extent)));
