    ...
    Simplify(bool r, const Scope<Interval> *bi, const Scope<ModulusRemainder> *ai) :
        ...
        for (Scope<Interval>::const_iterator iter = bi->cbegin(); iter != bi->cend(); ++iter) {
            ...
            if (const_int(iter.value().min, &i_min) &&
                ...
                bounds_info.push(iter.var(), make_pair(i_min, i_max));
            }
        }
...
private:
    ...
    void visit(const Mod *op, const Expr &e) {
        Expr a = mutate(op->a);
        Expr b = mutate(op->b);
        if (propagate_indeterminate_expression(a, b, op->type, &expr)) {
            return;
    ...
    void visit(const Min *op, const Expr &e) {
        Expr a = mutate(op->a);
        Expr b = mutate(op->b);
        if (propagate_indeterminate_expression(a, b, op->type, &expr)) {
            return;
    ...
    void visit(const Max *op, const Expr &e) {
    ...
    void visit(const EQ *op, const Expr &e) {
    ...
    void visit(const NE *op, const Expr &e) {
        expr = mutate(Not::make(op->a == op->b));
    }

    void visit(const LT *op, const Expr &e) {
        Expr a = mutate(op->a), b = mutate(op->b);
        if (propagate_indeterminate_expression(a, b, op->type, &expr)) {
            return;
        }
    ...
    void visit(const LE *op, const Expr &e) {
        expr = mutate(!(op->b < op->a));
    }

    void visit(const GT *op, const Expr &e) {
        expr = mutate(op->b < op->a);
    }

    void visit(const GE *op, const Expr &e) {
        expr = mutate(!(op->a < op->b));
    }

    void visit(const And *op, const Expr &e) {
        Expr a = mutate(op->a);
        Expr b = mutate(op->b);
        if (propagate_indeterminate_expression(a, b, op->type, &expr)) {
            return;
    ...
    void visit(const Or *op, const Expr &e) {
        Expr a = mutate(op->a), b = mutate(op->b);
        if (propagate_indeterminate_expression(a, b, op->type, &expr)) {
            return;
        }
    ...
    void visit(const Not *op, const Expr &e) {
        Expr a = mutate(op->a);
    ...
    void visit(const Select *op, const Expr &e) {
    ...
    void visit(const IfThenElse *op, const Stmt &s) {
        Expr condition = mutate(op->condition);
            ...
            if (const And *a = next.as<And>()) {
                ...
                if (eq && var) {
                    ...
                    if (!and_chain && eq->b.type().is_bool()) {
                        else_case = substitute(var, !eq->b, then_case);
                    }
                } else if (var) {
                    if (!or_chain) {
    ...
    void visit(const Load *op, const Expr &e) {
        ...
        if (const Broadcast *b = index.as<Broadcast>()) {
            Expr load = Load::make(op->type.element_of(), op->buffer_var, b->value);
            expr = Broadcast::make(load, b->lanes);
        } else if (index.same_as(op->index)) {
            expr = e;
        } else {
            expr = Load::make(op->type, op->buffer_var, index);
        }
    }
    ...
    void visit(const Call *op, const Expr &e) {
        ...
        if (op->call_type == Call::Halide) {
          // TODO(haichen): revisit here, for now don't increase ref count for bound vars
          /*
        ...
        if (op->is_intrinsic(Call::shift_left) ||
            ...
            if (const Load *first_load = new_args[0].as<Load>()) {
                ...
                for (Expr e : new_args) {
                    ...
                    if (load && equal(load->buffer_var, first_load->buffer_var)) {
                ...
                if ((int)load_indices.size() == terms) {
                    ...
                    if (interleaved_index.as<Ramp>()) {
                        ...
                        expr = Load::make(t, first_load->buffer_var, interleaved_index);
                        return;
                    }
                }
            }
          ...
          IRMutator::visit(op, e);
        }
    }

    template<typename T, typename Body>
    Body simplify_let(const T *op, Body b) {
      // TODO(haichen): maybe we don't need to this.
        internal_assert(!var_info.contains(op->var.get()))
            << "Simplify only works on code where every name is unique. Repeated name: " << op->var << "\n";
        ...
        if (new_value.same_as(value)) {
            ...
            debug(4) << "new let " << new_var << " = " << new_value << " in ... " << replacement << " ...\n";
        }
        ...
        if (new_value.defined() && no_overflow_scalar_int(new_value.type())) {
            ...
            if (const_int_bounds(new_value, &val_min, &val_max)) {
                bounds_info.push(new_var.get(), make_pair(val_min, val_max));
        ...
        if (no_overflow_scalar_int(value.type())) {
            ...
            if (const_int_bounds(value, &val_min, &val_max)) {
                bounds_info.push(op->var.get(), make_pair(val_min, val_max));
        ...
        if (info.old_uses > 0) {
            // The old var is still in use. We'd better keep it as well.
    ...
    void visit(const Let *op, const Expr &e) {
        if (simplify_lets) {
          expr = simplify_let<Let, Expr>(op, e);
        } else {
            IRMutator::visit(op, e);
        }
    }
    ...
    void visit(const LetStmt *op, const Stmt &s) {
        if (simplify_lets) {
          stmt = simplify_let<LetStmt, Stmt>(op, s);
        } else {
            IRMutator::visit(op, s);
        }
    }

    void visit(const AssertStmt *op, const Stmt &s) {
        IRMutator::visit(op, s);
    ...
    void visit(const For *op, const Stmt &s) {
        ...
        if (const_int(new_min, &new_min_int) &&
            ...
            bounds_info.push(op->loop_var.get(), make_pair(new_min_int, new_max_int));
        }
    ...
    void visit(const Provide *op, const Stmt &s) {
    ...
    void visit(const Store *op, const Stmt &s) {
        ...
        if (is_undef(value) || (load && equal(load->buffer_var, op->buffer_var) && equal(load->index, index))) {
            // foo[x] = foo[x] or foo[x] = undef is a no-op
            stmt = Evaluate::make(0);
        } else if (value.same_as(op->value) && index.same_as(op->index)) {
            stmt = s;
        } else {
            stmt = Store::make(op->buffer_var, value, index);
        }
    }

    void visit(const Allocate *op, const Stmt &s) {
    ...
    void visit(const ProducerConsumer *op, const Stmt &s) {
        Stmt body = mutate(op->body);
    ...
    void visit(const Block *op, const Stmt &s) {
